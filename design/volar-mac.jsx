// volar-mac.jsx — Mac desktop app: Today view with interactive hotkey + popover overlay
...
function TaskRow({ task, theme, isActive, onToggle, onSelect, hoverable = true }) {
  ...
  return (
    ...
    >
      ...
      >
        {task.done && <VolarIcon name="check" size={11} color="#fff" strokeWidth={2.6} />}
      </div>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <VolarIcon name="mic" size={28} color={a.solid} strokeWidth={1.6} />
      </div>
      <div style={{
...
function KeyBadge({ children, theme, accent = false }) {
  ...
  return (
    <span style={{
      ...
      fontFamily: VOLAR_MONO, fontSize: 10.5, fontWeight: 500,
...
function VolarMacApp({
  ...
  useEffectM(() => {
    if (focusOn && openTasks.length === 0) {
      setFocusOn(false);
      if (voiceFeedback) volarSpeak('All clear. Nothing left today.');
    }
  ...
  useEffectM(() => {
    ...
    const iv = setInterval(() => setFocusLeft(s => {
      if (s <= 1) {
        clearInterval(iv);
        setFocusOn(false);
        if (voiceFeedback) volarSpeak('Focus session complete. Nice work.');
        return 25 * 60;
      }
      return s - 1;
    }), 1000);
  ...
  const startFocus = () => {
    ...
    setFocusOn(true);
    if (voiceFeedback) volarSpeak('Focus session started. ' + (frogTask ? frogTask.title : 'Twenty five minutes.'));
  };
  const endFocus = () => { setFocusOn(false); setFocusLeft(25 * 60); volarStopSpeak(); };
  ...
  const completeFocusTask = (id) => {
    ...
    if (voiceFeedback) {
      volarSpeak(remaining > 0 ? `Done. ${remaining} left today.` : 'Done. All clear.');
    }
  };

  const readDay = () => {
    const open = [...nowTasks, ...laterTasks];
    if (open.length === 0) { volarSpeak('All clear. Nothing scheduled.'); return; }
    const list = open.slice(0, 3).map(t => t.title).join('. ');
    volarSpeak(`You have ${open.length} open task${open.length === 1 ? '' : 's'}. Up next: ${list}.`);
  };
  ...
  const saveFromPopover = () => {
    if (voiceFeedback) {
      ...
      volarSpeak(`Added. ${ex.title}. ${ex.when.replace('·', ',')}.`);
    }
    ...
    <div ref={rootRef} tabIndex={0} style={{
      ...
      <div style={{ display: 'flex', height: `calc(100% - 38px)`, position: 'relative', zIndex: 1 }}>
        ...
        <div style={{
          ...
          <div style={{ padding: '12px 10px 4px' }}>
            ...
            >
              <VolarIcon name="mic" size={13} color={a.solid} strokeWidth={1.8} />
            ...
            {focusOn ? (
              <div style={{
                ...
                <span style={{
                  ...
                  animation: focusPaused ? 'none' : 'volar-pulse 1.6s ease-in-out infinite',
                }} />
                ...
                <span style={{
                  fontFamily: VOLAR_MONO, fontSize: 13, fontWeight: 600, color: a.solid,
                ...
                <span onClick={() => setFocusPaused(p => !p)} style={{
                  ...
                  <VolarIcon name={focusPaused ? 'play' : 'pause'} size={10} color={c.textPri} strokeWidth={2} />
                </span>
                <span onClick={endFocus} style={{
                  ...
                  <VolarIcon name="stop" size={9} color={c.textSec} />
                </span>
              </div>
            ) : (
              <div style={{
                ...
                <span onClick={startFocus} title="Start a 25-minute focus session" style={{
                  ...
                  <VolarIcon name="play" size={8} color="#fff" />
                  <span>Focus</span>
                </span>
              </div>
            )}
      ...
      {popoverState && (
        <div style={{
          ...
          paddingTop: 60,
          zIndex: 10,
          animation: 'volar-fade .12s ease-out',
      ...
      <style>{`
        @keyframes volar-fade { from { opacity: 0 } to { opacity: 1 } }
        @keyframes volar-pulse { 0%,100% { opacity: 1 } 50% { opacity: 0.35 } }
      `}</style>
    </div>
  );
}
...
function ToolButton({ icon, accent = false, activeTint = false, onClick, title, theme }) {
  ...
  return (
    ...
    >
      <VolarIcon name={icon} size={14} color={fg} strokeWidth={1.8} />
    </button>
  );
}

Object.assign(window, { VolarMacApp, TaskRow, SidebarItem, SectionLabel, KeyBadge, TrafficLights, EmptyToday, ToolButton });
