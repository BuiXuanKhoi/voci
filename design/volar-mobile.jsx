// volar-mobile.jsx — iOS companion: same product, mobile form
...
function StatusBar({ time = '9:41', dark = true }) {
  ...
  return (
    <div style={{
      ...
      fontFamily: VOLAR_FONT, fontSize: 16, fontWeight: 500,
  ...
  return (
    <div style={{
      ...
      <div onClick={onToggle} style={{
        ...
        {task.done && <VolarIcon name="check" size={13} color="#fff" strokeWidth={2.6} />}
      </div>
    ...
    <div style={{
      ...
      {tabs.map(t => (
        <div key={t.id} onClick={() => onChange(t.id)} style={{
          ...
          <VolarIcon name={t.icon} size={22} color={active === t.id ? a.solid : c.textSec} strokeWidth={active === t.id ? 2 : 1.6} />
...
function MobileVoiceSheet({ theme, onClose, animatedWave = true, transcriptIndex = 0 }) {
  ...
  return (
    <div style={{
      ...
      zIndex: 50,
      animation: 'volar-fade-in .18s ease-out',
    }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        animation: 'volar-slide-up .28s cubic-bezier(.2,.7,.3,1)',
      }}>
        {/* grabber */}
        <div style={{
          ...
          <div style={{
            ...
            {state === 'recording' && (
              <span style={{
                ...
                animation: 'volar-pulse-m 1.2s ease-out infinite',
              }} />
            )}
            <span>
        ...
        {state === 'done' && (
          <div style={{ display: 'flex', justifyContent: 'center', padding: '8px 0 18px' }}>
            <div style={{
              ...
              <VolarIcon name="check" size={28} color="#0e2a16" strokeWidth={2.6} />
            </div>
          </div>
        )}
          ...
          {state === 'recording' ? (
            <>
              ...
              <span style={{
                ...
                animation: 'volar-caret 0.9s steps(2, start) infinite',
              }} />
            </>
          ) : example.sentence}
        </div>
        ...
        {(state === 'parsed' || state === 'saving' || state === 'done') && (
          <div style={{
            ...
            animation: 'volar-rise .35s cubic-bezier(.2,.7,.3,1) both',
          }}>
        ...
        {(state === 'parsed' || state === 'saving') && (
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={onClose} style={{
              ...
              border: `0.5px solid ${c.border}`,
              color: c.textPri, fontSize: 15, fontWeight: 500, fontFamily: VOLAR_FONT,
            }}>Cancel</button>
            <button onClick={save} style={{
              ...
              fontSize: 16, fontWeight: 500, fontFamily: VOLAR_FONT,
      ...
      <style>{`
        @keyframes volar-fade-in { from { opacity: 0 } to { opacity: 1 } }
        @keyframes volar-slide-up { from { transform: translateY(100%) } to { transform: translateY(0) } }
        @keyframes volar-pulse-m {
...
function MicButton({ theme, onPress, big = true }) {
  ...
  return (
    <div style={{
      ...
      >
        <VolarIcon name="mic" size={30} color="#fff" strokeWidth={1.8} />
      </button>
    </div>
  );
}
...
// ── Main mobile app ─────────────────────────────────────────────────────────
function VolarMobileApp({ theme, width = 390, height = 844, initialEmpty = false, animatedWave = true }) {
        ...
        {initialEmpty ? (
          <div style={{ padding: 36, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14, marginTop: 60 }}>
            <div style={{
              ...
              <VolarIcon name="mic" size={36} color={a.solid} strokeWidth={1.6} />
            </div>
          ...
          <>
            ...
            <div style={{
              ...
              <span>Long-press the mic or say <span style={{ color: a.solid, fontWeight: 500 }}>"Hey Volar"</span> from anywhere.</span>
            </div>
          </>
        )}
      </div>
...
Object.assign(window, { VolarMobileApp, StatusBar, MobileTaskCard, MobileVoiceSheet, MicButton, MobileTabBar });
