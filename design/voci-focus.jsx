// voci-focus.jsx — fullscreen one-task Focus mode with task navigation

const { useState: useStateF, useEffect: useEffectF } = React;

function VociFocusOverlay({
  theme,
  openTasks,
  idx, setIdx,
  ...
  return (
    <div style={{
      ...
      animation: 'voci-fade .18s ease-out',
    }}>
      ...
      <div style={{
        fontFamily: VOCI_MONO, fontSize: 76, fontWeight: 600, lineHeight: 1,
      ...
      <button onClick={() => onCompleteTask(task.id)} style={{
        ...
        <VocIcon name="check" size={13} color="#fff" strokeWidth={2.4} />
        <span>Mark done</span>
      </button>
...
function FocusRoundBtn({ theme, icon, onClick, title, disabled = false }) {
  ...
  return (
    ...
    >
      <VocIcon name={icon} size={12} color="rgba(255,255,255,0.7)" strokeWidth={1.8} />
    </button>
  );
}

Object.assign(window, { VociFocusOverlay });
