// volar-popover.jsx — animated quick capture popover
...
function VolarPopover({
  ...
  return (
    <div style={{
          ...
          {state === 'recording' && (
            <>
              <span style={{
                ...
                animation: 'volar-pulse 1.2s ease-out infinite',
              }} />
              <span>Listening…</span>
            </>
          )}
          ...
          <div style={{ height: 42, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {state === 'done' ? (
              <div style={{
                ...
                <VolarIcon name="check" size={18} color="#0e2a16" strokeWidth={2.4} />
              </div>
            ) : state === 'error' ? (
      ...
      <style>{`
        @keyframes volar-pulse {
          0%   { box-shadow: 0 0 0 0 ${c.high}80; }
          70%  { box-shadow: 0 0 0 6px ${c.high}00; }
          100% { box-shadow: 0 0 0 0 ${c.high}00; }
        }
        @keyframes volar-caret { 0%, 50% { opacity: 1 } 50.01%, 100% { opacity: 0 } }
        @keyframes volar-rise {
...
function Kbd({ children, theme }) {
  return (
    <span style={{
      ...
      fontFamily: VOLAR_MONO, fontSize: 10, fontWeight: 500,
    }}>{children}</span>
  );
}
...
function Spinner({ color = '#fff', size = 14 }) {
  return (
    <span style={{
      ...
      animation: 'volar-spin 0.7s linear infinite',
    }}>
      <style>{`@keyframes volar-spin { to { transform: rotate(360deg); } }`}</style>
    </span>
  );
}

Object.assign(window, { VolarPopover, Waveform, PriorityBadge, TimeBadge, Kbd, Spinner, ParseRow });
