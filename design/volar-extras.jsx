// volar-extras.jsx — onboarding, settings, notification, menu bar icon

const { useState: useStateX, useEffect: useEffectX } = React;

// ─── Menu bar icon (Mac) ─────────────────────────────────────────────────────
function VolarMenuBar({ theme, state = 'all' /* idle | active | focuslock | all */ }) {
  const { c, a } = theme;
  // Render a fake "macOS menu bar slice" with the Volar icon in different states
  const Slice = ({ mode }) => {
    ...
    return (
      <div style={{
        ...
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
          <div style={{ position: 'relative', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 18, height: 18 }}>
            {active && (
              <span style={{
                ...
                animation: 'volar-mic-pulse 1.6s ease-in-out infinite',
              }} />
            )}
            <VolarIcon name="mic" size={14} color={active ? a.solid : 'rgba(255,255,255,0.7)'} strokeWidth={active ? 2 : 1.6} />
          ...
          {active && (
            <span style={{
              fontSize: 11, fontFamily: VOLAR_MONO, fontWeight: 500,
          ...
          {focus && (
            <span style={{
              ...
              <span style={{ fontFamily: VOLAR_MONO, color: a.solid }}>42:18</span>
            </span>
          )}
        </div>
  ...
  return (
    <div style={{
      ...
      fontFamily: VOLAR_FONT, color: c.textPri,
      borderRadius: 12,
...
// ─── Onboarding ──────────────────────────────────────────────────────────────
function VolarOnboardingStep({ theme, step = 1 /* 1 | 2 | 3 */, width = 720, height = 480 }) {
  ...
  return (
    <div style={{
          ...
          {step === 1 && (
            <>
              ...
              <div style={{
                ...
                <VolarIcon name="mic" size={42} color="#fff" strokeWidth={1.8} />
              </div>
              ...
              <div style={subStyle}>
                Volar is a voice-first task manager. No typing, no menus — just hold the hotkey from anywhere on your Mac and say what you need to do.
              </div>
              <button style={{
          ...
          {step === 2 && (
            <>
              <div style={{
                ...
                <VolarIcon name="mic" size={42} color={a.solid} strokeWidth={1.8} />
                {/* concentric rings */}
                {[1,2,3].map(i => (
                  <span key={i} style={{
              ...
              <div style={titleStyle}>Volar needs your microphone.</div>
              <div style={subStyle}>
          ...
          {step === 3 && (
            <>
              <div style={{
                ...
                animation: 'volar-kbd-pulse 1.6s ease-in-out infinite',
              }}>
      ...
      <div style={{
        ...
        <span style={{ color: c.textSec }}>volar.app</span>
      </div>
    </div>
  );
}
...
// ─── Settings window ─────────────────────────────────────────────────────────
function VolarSettings({ theme, width = 620, height = 480, initialTab = 'general' }) {
  const { c, a, font } = theme;
        ...
        <div style={{ display: 'flex', justifyContent: 'center', padding: '4px 14px 10px', gap: 4 }}>
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{
              ...
              <VolarIcon name={t.icon} size={18} color={tab === t.id ? a.solid : c.textSec} strokeWidth={1.6} />
              {t.label}
            </button>
          ))}
        </div>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '22px 32px' }}>
        {tab === 'general' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <SettingsRow theme={theme} label="Launch at login"
              hint="Volar starts in the background and lives in your menu bar.">
            ...
            <SettingsRow theme={theme} label="Hyperfocus interrupt after"
              hint="Volar checks in if you've been deep on one task this long.">
        ...
        {tab === 'hotkeys' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <SettingsRow theme={theme} label="Show Volar window"
              hint="Bring the main window to the front.">
              <KeyRecorder theme={theme} keys={['⌃', '⌥', 'V']} />
            </SettingsRow>
            <SettingsRow theme={theme} label="Toggle Focus Lock"
              hint="Lock the current task as your only focus — Volar will gently interrupt if you drift.">
        ...
        {tab === 'appear' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <SettingsRow theme={theme} label="Theme" hint="Volar is dark-only — the way Mac power users live.">
            ...
            <SettingsRow theme={theme} label="Accent color" hint="Used for active states and the capture button.">
              <div style={{ display: 'flex', gap: 10 }}>
                {Object.entries(VOLAR_ACCENTS).map(([k, v]) => (
                  <span key={k} style={{
        ...
        {tab === 'about' && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14, padding: 16 }}>
            <div style={{
              ...
              <VolarIcon name="mic" size={32} color="#fff" strokeWidth={1.8} />
            </div>
            <div style={{ fontSize: 22, fontWeight: 500, letterSpacing: '-0.02em' }}>Volar 1.0.2</div>
    ...
    <div style={{
      ...
      {options.map(o => (
        <button key={o.id} onClick={() => !o.disabled && onChange(o.id)} style={{
          ...
          border: 'none', fontSize: 11.5, fontWeight: 500, fontFamily: VOLAR_FONT,
...
function VolarNotification({ theme, width = 380, height = 150 }) {
  const { c, a, font } = theme;
  return (
    <div style={{
      width, height,
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{
          ...
          <VolarIcon name="mic" size={20} color="#fff" strokeWidth={1.8} />
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
            <span style={{ fontSize: 13, fontWeight: 500, color: c.textPri }}>Volar</span>
...
function VolarEmptyArtboard({ theme, width = 460, height = 380 }) {
  const { c, font } = theme;
  return (
    <div style={{
      width, height,
...
// ─── Morning Frog Prompt ─────────────────────────────────────────────────────
function VolarMorningFrog({ theme, width = 720, height = 580 }) {
  const { c, a, font } = theme;
  const candidates = [
      ...
      <div style={{
        ...
        <div style={{
          ...
          <button style={{
            ...
            animation: 'volar-frog-pulse 2.4s ease-in-out infinite',
          }}>
            <VolarIcon name="mic" size={16} color={a.solid} strokeWidth={1.8} />
            ...
            {candidates.map(c2 => (
              <div key={c2.id} onClick={() => setPicked(c2.id)} style={{
                ...
                <span style={{
                  ...
                  {picked === c2.id && <VolarIcon name="check" size={9} color="#fff" strokeWidth={2.6} />}
                </span>
                <span style={{
...
function VolarTaskBreakdown({ theme, width = 480, height = 540 }) {
  const { c, a, font } = theme;
  const steps = [
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
        ...
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6,
          ...
          <VolarIcon name="sparkle" size={11} color={a.solid} strokeWidth={1.8} />
          AI
        </div>
      </div>
...
Object.assign(window, {
  VolarMenuBar, VolarOnboardingStep, VolarSettings, VolarNotification, VolarEmptyArtboard,
  VolarMorningFrog, VolarTaskBreakdown,
