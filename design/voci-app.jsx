// voci-app.jsx — entry, wires DesignCanvas + all artboards + Tweaks panel
...
function App() {
  ...
  return (
    <>
      <DesignCanvas>
        {/* ─── HERO: Mac prototype ──────────────────────────────────────── */}
        <DCSection id="mac-proto" title="Voci for Mac" subtitle="Interactive prototype · hold ⌃⌥Space to capture by voice · ambient focus background + sound (🔊 in title bar) · waveform button reads your day aloud">
          <DCArtboard id="mac-today" label="Today view · click anywhere to interact" width={920} height={580}>
            <VociMacApp theme={theme} initialEmpty={t.emptyState} animatedWave={t.animatedWave}
          ...
          <DCArtboard id="mac-empty" label="Empty state · all clear" width={920} height={580}>
            <VociMacApp theme={theme} initialEmpty animatedWave={t.animatedWave} enableHotkey={false}
              ambient={t.ambient} ambientImage={t.ambientImage} voiceFeedback={t.voiceFeedback} motionIntensity={t.motion / 100} />
          </DCArtboard>
        </DCSection>
        ...
        <DCSection id="popover-states" title="Quick capture popover" subtitle="Anatomy of the 5 popover states — the screen users see 10–20× a day">
          <DCArtboard id="pop-rec" label="1 · Recording" width={400} height={300}>
            <PopoverFrame theme={theme}>
              <VociPopover theme={theme} state="recording" transcriptIndex={t.transcriptIndex} animatedWave={t.animatedWave} />
            </PopoverFrame>
          </DCArtboard>
          <DCArtboard id="pop-parsed" label="2 · Parsed" width={400} height={440}>
            <PopoverFrame theme={theme}>
              <VociPopover theme={theme} state="parsed" transcriptIndex={t.transcriptIndex} animatedWave={t.animatedWave} />
            </PopoverFrame>
          </DCArtboard>
          <DCArtboard id="pop-saving" label="3 · Saving" width={400} height={440}>
            <PopoverFrame theme={theme}>
              <VociPopover theme={theme} state="saving" transcriptIndex={t.transcriptIndex} animatedWave={t.animatedWave} />
            </PopoverFrame>
          </DCArtboard>
          <DCArtboard id="pop-done" label="4 · Saved" width={400} height={340}>
            <PopoverFrame theme={theme}>
              <VociPopover theme={theme} state="done" transcriptIndex={t.transcriptIndex} animatedWave={t.animatedWave} />
            </PopoverFrame>
          </DCArtboard>
          <DCArtboard id="pop-error" label="5 · Error" width={400} height={260}>
            <PopoverFrame theme={theme}>
              <VociPopover theme={theme} state="error" transcriptIndex={t.transcriptIndex} animatedWave={t.animatedWave} />
            </PopoverFrame>
          </DCArtboard>
        </DCSection>
        ...
        <DCSection id="settings" title="Settings" subtitle="Native macOS pattern — tabs across the top, plain rows below.">
          <DCArtboard id="set-gen" label="General" width={620} height={640}>
            <VociSettings theme={theme} initialTab="general" />
          </DCArtboard>
          <DCArtboard id="set-hot" label="Hotkeys" width={620} height={520}>
            <VociSettings theme={theme} initialTab="hotkeys" />
          </DCArtboard>
        </DCSection>
        ...
        <DCSection id="out-of-scope" title="Parked · iOS companion" subtitle="v1.0 explicitly excludes iOS. Keeping the work here in case scope opens up later.">
          <DCArtboard id="mob-today" label="iPhone · Today" width={390} height={844}>
            <VociMobileApp theme={theme} animatedWave={t.animatedWave} />
          </DCArtboard>
          <DCArtboard id="mob-empty" label="iPhone · Empty" width={390} height={844}>
            <VociMobileApp theme={theme} initialEmpty animatedWave={t.animatedWave} />
          </DCArtboard>
        </DCSection>
      </DesignCanvas>
      ...
      <TweaksPanel>
        ...
        <TweakColor label="Accent" value={theme.a.solid}
          options={Object.values(VOCI_ACCENTS).map(v => v.solid)}
          onChange={(hex) => {
            const key = Object.entries(VOCI_ACCENTS).find(([_, v]) => v.solid === hex)?.[0] || 'indigo';
            setTweak('accent', key);
          }} />
