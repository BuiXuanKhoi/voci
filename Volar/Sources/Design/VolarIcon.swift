// Sources/Design/VolarIcon.swift — icon name catalog + SF Symbols rendering (spec §3)
import SwiftUI
...
/// `VolarIcon(_ name:size:color:weight:)` — the one place views reach for an icon glyph.
struct VolarIcon: View {
    let name: VolarIconName
    var size: CGFloat
    var color: Color
    var weight: Font.Weight

    init(_ name: VolarIconName, size: CGFloat = 14, color: Color = VolarColor.textPri, weight: Font.Weight = .regular) {
        self.name = name
        self.size = size
        self.color = color
        self.weight = weight
