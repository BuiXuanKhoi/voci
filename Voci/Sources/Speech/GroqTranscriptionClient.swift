...
/// in the app binary: in production this returns the Voci **proxy** base URL + a short-lived
...
/// Keychain/StoreKit-backed provider returning the Voci proxy URL + a per-user token.
///
/// - Base URL: `GROQ_BASE_URL` env or `voci.groqBaseURL` default; falls back to Groq direct
///   (`https://api.groq.com/openai/v1`) so a developer can smoke-test without a proxy.
/// - Token: `GROQ_PROXY_TOKEN` or `GROQ_API_KEY` env, or `voci.groqToken` default. Absent → throws
///   `.missingCredentials`.
struct EnvironmentGroqCredentialProvider: GroqCredentialProvider {
    ...
    func baseURL() async throws -> URL {
        if let s = ProcessInfo.processInfo.environment["GROQ_BASE_URL"]
            ?? UserDefaults.standard.string(forKey: "voci.groqBaseURL"),
    ...
    func authorization() async throws -> String? {
        ...
        guard let token = env["GROQ_PROXY_TOKEN"] ?? env["GROQ_API_KEY"]
                ?? UserDefaults.standard.string(forKey: "voci.groqToken"),
              !token.isEmpty else {
...
struct GroqTranscriptionClient: Sendable {
    ...
    func transcribe(audio: Data, filename: String, model: GroqModel) async throws -> String {
        ...
        let boundary = "voci-\(UUID().uuidString)"
