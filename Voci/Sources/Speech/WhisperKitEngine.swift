...
final class WhisperKitEngine: SpeechEngine {
    ...
    func start(onPartial: @escaping (String) -> Void) {
        ...
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("voci-whisper-\(UUID().uuidString).m4a")
        ...
        do {
            ...
            guard rec.record() else {
                onError?(NSError(domain: "Voci.WhisperKit", code: 0, userInfo: [NSLocalizedDescriptionKey: "Recorder failed to start"]))
                return
            }
            recorder = rec
            fileURL = url
        ...
        Task { @MainActor [weak self] in
            ...
            guard let pipe = self.whisperKit else {
                if self.session == token {
                    self.onError?(NSError(domain: "Voci.WhisperKit", code: 1, userInfo: [NSLocalizedDescriptionKey: "Model not loaded"]))
                }
            ...
            do {
                ...
                if text.isEmpty {
                    self.onError?(NSError(domain: "Voci.WhisperKit", code: 2, userInfo: [NSLocalizedDescriptionKey: "Nothing recognized"]))
                } else {
                    self.onFinal?(text)
                }
            } catch {
