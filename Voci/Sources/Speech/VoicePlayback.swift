...
/// Wraps `AVSpeechSynthesizer` for spoken feedback — mirrors `vociSpeak`/`vociStopSpeak` in
/// `voci-ambient.jsx` plus the `readDay` behavior from `voci-mac.jsx`.
@MainActor
final class VoicePlayback {
    private let synthesizer = AVSpeechSynthesizer()

    /// `vociSpeak(text)`. The JS prototype sets `utterance.rate = 1.03` against the Web Speech
    ...
    /// Mirrors `voci-mac.jsx`'s `readDay`: announces the open-task count and up to the first 3
