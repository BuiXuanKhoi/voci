...
// schema `[VociTask.self, CompletionEvent.self, ParseCorrection.self]`). This task owns
...
final class ReminderScheduler: NSObject {
    ...
    /// `VociCore.nextResurfaceDate`). Scheduled ahead like a normal reminder — not fired
    /// immediately.
    ...
    /// (`VociTask.isSensitive` is deliberately not carried on `TaskItem` — see that field's doc
    /// comment in `VociTask.swift`).
    ...
    private static func makeContext() -> ModelContext {
        ...
        let configuration = ModelConfiguration("VociReminders", schema: schema)
        ...
        let fallback = ModelConfiguration("VociReminders-fallback", schema: schema, isStoredInMemoryOnly: true)
    ...
    // Voci` gives `ReminderSchedulerTests` access to `internal` symbols only, and these three are
