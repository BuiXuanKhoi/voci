...
// `VociTask` (see VociTask.swift) — this file only defines the pure value types + pure math.
import Foundation
import VociCore
...
enum RecurrenceEngine {
    ...
    /// The persisted fields a reset touches — deliberately narrow (not the whole `VociTask`) so
    ...
    static func reset(
        ...
        let nextConditions = conditions.map { condition -> VociCore.Condition in
            switch condition {
