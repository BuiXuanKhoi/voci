// Sources/Model/VociTask.swift — SwiftData-persisted task (v2 shape), mapped to/from TaskItem.
//
...
// container from a bare `Schema([VociTask.self])`. Introducing a versioned schema now, purely to
...
/// Wire-format mirror of `VociCore.Condition` for JSON persistence. Hand-rolled (rather than
/// requiring `VociCore.Condition` to be `Codable`, which the frozen contract does not promise, or
/// relying on `Codable` synthesis for an enum with associated values) so the on-disk shape is
/// explicit and stable. Case names/payloads match `contracts/vocicore-api.md` 1:1 by inspection —
...
private struct ConditionDTO: Codable {
    ...
    init(_ condition: VociCore.Condition) {
        switch condition {
    ...
    /// invalidating the whole `conditions` array (see `VociTask.conditions` getter below).
    var asCondition: VociCore.Condition? {
        switch kind {
...
final class VociTask {
    ...
    var conditions: [VociCore.Condition] {
        ...
        set {
            ...
            conditionsData = (try? JSONEncoder().encode(dtos)) ?? VociTask.emptyJSONArray
        }
    }
