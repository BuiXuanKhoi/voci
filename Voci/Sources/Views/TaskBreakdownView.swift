...
struct TaskBreakdownView: View {
    ...
    private var header: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            Spacer(minLength: 8)
            HStack(spacing: 6) {
                VocIcon(.sparkle, size: 11, color: accentColors.solid, weight: .regular)
                Text("AI")
            }
            .font(.system(size: 11, weight: .medium))
        ...
        HStack(spacing: 12) {
            VStack(spacing: 2) {
                ForEach(0..<3, id: \.self) { _ in
                    Rectangle().fill(VociColor.textSec).frame(width: 12, height: 1)
                }
            }
            .opacity(0.35)
            ...
            Text(step.duration)
                ...
                .clipShape(Capsule())
                .overlay(Capsule().stroke(VociColor.border, lineWidth: 0.5))
        }
