...
struct TaskDetailView: View {
            ...
            ScrollView {
                Group {
                    if task.details.isEmpty {
                        Text("No description")
                            .foregroundStyle(VociColor.textMut.opacity(0.7))
                    } else {
                        Text(task.details)
                            .foregroundStyle(VociColor.textSec)
                    }
    ...
    private func readButton(_ task: TaskItem) -> some View {
        Button {
            ...
            HStack(spacing: 8) {
                VocIcon(.volume, size: 13, color: accentColors.solid, weight: .regular)
        ...
        HStack(spacing: 8) {
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 9, style: .continuous)
                    .stroke(VociColor.high.opacity(0.25), lineWidth: 0.5)
            )
