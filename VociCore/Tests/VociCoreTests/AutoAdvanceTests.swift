...
/// re-invoking `nextTask(from:now:calendar:)` with an updated snapshot in which the previously
...
struct AutoAdvanceTests {
    ...
    func completingActiveTaskAdvancesToNextTask() {
        ...
        let before = nextTask(from: [activeTask, nextEligibleTask], now: referenceNow, calendar: testCalendar)
        #expect(before?.id == activeTask.id)

        var completedActiveTask = activeTask
        completedActiveTask.status = .done

        let after = nextTask(from: [completedActiveTask, nextEligibleTask], now: referenceNow, calendar: testCalendar)
    ...
    func completingSoleEligibleTaskReturnsNil() {
        ...
        let before = nextTask(from: [onlyTask], now: referenceNow, calendar: testCalendar)
        #expect(before?.id == onlyTask.id)

        var completedOnlyTask = onlyTask
        completedOnlyTask.status = .done

        let after = nextTask(from: [completedOnlyTask], now: referenceNow, calendar: testCalendar)
