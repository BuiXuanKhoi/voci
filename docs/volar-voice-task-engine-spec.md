# Volar — Voice Task Engine Specification
...
**Audience:** AI coding agent implementing inside the existing Volar codebase.

---

## 1. Product context

Volar is a voice-first, ADHD-first task manager for macOS. Global hotkey `⌃⌥Space` opens quick capture; the menu bar shows exactly **one** active task. This spec covers the voice → task intelligence layer:
...
Static scheduled notifications can't check task state at fire time. Volar runs a persistent menu bar process → use an internal scheduler.
