...
- **FR-011c (capture-time conflict advisory, added 2026-07-16)**: When a new task is confirmed, BEFORE saving, the system MUST run a deterministic conflict check of the candidate against the current task snapshot + calendar busy-intervals and, ONLY when it finds a high-signal conflict, surface it as ONE calm advisory line on the confirm card. The advisory is NEVER a blocker — the user can ignore it (Enter saves as-is) or act on it (reschedule / adjust) — and zero-conflict captures MUST show nothing (no friction, constitution V). Detected conflict kinds: (a) **deadline-capacity** — the candidate's deadline window is already over-committed by existing near-deadline tasks' estimates plus calendar-busy time ("3 tasks already due tomorrow, ~5h — this may not fit"); (b) **deadline-collision** with an existing urgent / today's-most-important task; (c) **depends-on-blocked** — the candidate would depend on a task that is itself overdue/blocked/stuck; (d) **competes-with-frog** — a new high-priority task rivals today's chosen most-important task; (e) **possible-duplicate** of an existing open task (fuzzy title match). The conflict computation MUST be a PURE function of its inputs (constitution III) — calendar busy-intervals are passed in, never read inside the engine; the advisory MUST NOT auto-modify or auto-attach anything (constitution II).
...
- **FR-014b (spoken delivery, added 2026-07-16)**: Reminders MUST support an on-device spoken (text-to-speech) delivery channel IN ADDITION TO — never replacing — the visual notification. A global setting selects the channel: visual-only / visual+voice (default) / voice-only. Voice is a HIGH rung of the escalation ladder: the system speaks only for high-urgency reminders or after a visual reminder went unacknowledged — it does not narrate every event. Spoken copy MUST be one short, calm sentence (e.g. "In 10 minutes: design review."). TTS MUST run on-device (no audio/text egress — constitution I). **Context gating**: voice MUST be suppressed when a calendar event shows the user is in a meeting, a call/mic capture is active, the screen is being shared, Do Not Disturb is on, or other audio is playing; when suppressed, the visual notification still fires. **Privacy**: a task marked sensitive MUST be spoken only as a generic phrase ("You have a reminder"), with detail shown visually — never read its title aloud. The same gated on-device spoken channel MAY announce orchestrator events (e.g. an agent finishing), under the same suppression rules.
